...
# 0.1 Message Delivery Guarantee Semantics 

- At most once
- At least once
- Exactly once

首先，之多一次一般不用。那么消息安全机制可选择至少一次和刚好一次这两种策略。默认情况写，当生产者出现异常，会进行消息重传，保证至少一次。那么恰好一次，有两种实现方案，一种是每个分区只有一个生产者，当生产者异常时，读取最后一条消息，决定是否重传。另一种是生产者保证至少一次，消费者根据主键去重。
